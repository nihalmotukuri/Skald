...
router.put('/remove_tasklist', async (req, res) => {
  ...
  try {
    const updatedUser = User.findOneAndUpdate({ firebaseUid }, { $pull: {taskList: taskListToRemove} }, { new: true })
    console.log(updatedUser)
    res.status(201).json({ message: "Task list successfully added", taskListToRemove })
  } catch (err) {
    res.status(500).json({ message: err.message })
  }
})
