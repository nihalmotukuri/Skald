...
router.get("/remove_tasklist", async (req, res) => {
  const { firebaseUid, taskList } = req.body

  try {
    const updatedTasks = findOneAndUpdate({ firebaseUid, taskList }, { taskList: '' }, { new: true })
    console.log(updatedTasks)
    res.status(201).json({ message: "Task list successfully added", updatedTasks })
  } catch (err) {
    res.status(500).json({ error: error.message });
  }
})
