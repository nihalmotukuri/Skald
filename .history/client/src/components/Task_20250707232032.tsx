...
import { HiChevronDown, HiChevronUp } from "react-icons/hi2"import { ImStopwatch } from "react-icons/im"
...
const Task = ({ task, toggleTask }: TaskProps) => {
            ...
            case "done":
                return (
                    <p className="text-[14px] font-medium text-white flex items-center">
                        <SiTicktick className="mr-[8px]" /> Done
                    </p>
                );
            case "in-progress":
                return (
                    <p className="text-[14px] font-medium text-white flex items-center">
                        <CiStopwatch className="mr-[6px] text-[22px]" /> In Progress
                    </p>
                );
            case "pending":
                return (
    ...
    return (
        <li className={`${openTask ? "auto" : "h-[58px]"} flex flex-col items-start justify-between bg-white/2 backdrop-blur-md border border-white/6 rounded-md shadow-lg p-4 overflow-hidden`}>
