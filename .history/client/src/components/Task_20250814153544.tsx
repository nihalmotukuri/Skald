...
const TaskItem = ({ task, toggleTask, setDisplayEditTask, setEditTaskId }: TaskProps) => {
    ...
    return (
        <li className={`flex flex-col items-start justify-between ${!task.completed ? (isDark ? "bg-white/2" : "bg-neutral-300") : (isDark ? "bg-white/6" : "bg-neutral-300")} backdrop-blur-md border rounded-md p-4 overflow-hidden ${task.status === "Cancelled" ? "opacity-50" : ""} ${formatDueDate(task.date!, task.time!, task.completed!).includes('Overdue') ? "border-red-900/40 !bg-red-900/4" : "border-white/6"}`}>
