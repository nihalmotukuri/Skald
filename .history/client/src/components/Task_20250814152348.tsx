...
const TaskItem = ({ task, toggleTask, setDisplayEditTask, setEditTaskId }: TaskProps) => {
    ...
    return (
        <li className={`flex flex-col items-start justify-between ${!task.completed && isDark ? "bg-white/2" : "bg-white/6"} ${!task.completed && i!isDark ? "text-slate-400 border-white/10 bg-white/2" : "text-[#2d3748] border-black/20 bg-neutral-200"} backdrop-blur-md border rounded-md shadow-lg p-4 overflow-hidden ${task.status === "Cancelled" ? "opacity-50" : ""} ${formatDueDate(task.date!, task.time!, task.completed!).includes('Overdue') ? "border-red-900/40 !bg-red-900/4" : "border-white/6"}`}>
