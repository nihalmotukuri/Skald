...
const TaskItem = ({ task, toggleTask, setDisplayEditTask, setEditTaskId }: TaskProps) => {
    ...
    const taskStatus = () => {
        switch (task.status) {
            case "Done":
                return <p className={`text-[14px] font-medium ${isDark ? "text-white" : "text-black/60"} flex items-center`}>
                    <SiTicktick className="mr-[8px] text-[18px]" /> Done
                </p>;
            case "In Progress":
                return <p className="text-[14px] font-medium text-white flex items-center">
