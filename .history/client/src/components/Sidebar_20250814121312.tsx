...
const Sidebar = ({ user }: {user: User}) => {
  ...
  const openSidebar = () => (
    <motion.div
      className={`min-w-[260px] rounded-[18px] p-[8px] flex flex-col justify-between max-h-full ${isDark ? "bg-white/5" : "bg-black/20 text-black"} backdrop-blur-md border border-white/10 rounded-2xl shadow-lg`}
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
