...
const AddTaskDialog = ({ setDisplayAddTask }: AddTaskDialogProps) => {
    ...
    return (
        <section className={`fixed top-0 left-0 z-999 ${isDark ? "bg-black/40" : "bg-gray-300/30" } h-full w-full flex justify-center items-center`}>
            <form
                onSubmit={onAddTask}
                className={`${isDark ? "bg-white/6 border-white/10" : "bg-black/4 border-black/30"} shadow-lg backdrop-blur-xl border rounded-md p-[24px] w-[420px] flex flex-col gap-[18px] relative`}
            >
                <div
                ...
                <button
                    type='submit'
                    className={`${isDark ? "bg-white/5 border-white/10 hover:bg-white/10" : "bg-neutral-100/20 !border-black/30 hover:bg-neutral-200"} backdrop-blur-md border rounded-md text-[12px] px-[12px] py-[6px] w-full cursor-pointer`}
                >
                    Add Task
                </button>
            </form>
