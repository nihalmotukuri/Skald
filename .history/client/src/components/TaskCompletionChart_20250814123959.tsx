...
const TaskCompletionChart = ({ total, completed }: TaskCompletionChartProps) => {
  const { isDark } = useSelector((store: RootState) => store.)
  const percentage = total > 0 ? Math.round((completed / total) * 100) : 0;

  const startAngle = 90; // Top
  const endAngle = 90 + (percentage / 100) * 360; // Anti-clockwise
  ...
  const renderCenterLabel = () => (
    <foreignObject x="30" y="60" width="140" height="80">
      <div className="flex flex-col items-center justify-center w-full h-full text-white font-mono ml-[10px]">
        <span className="text-4xl font-bold mt-[18px]">{percentage}%</span>
  ...
  return (
    <motion.div className="h-full w-full">
      <Card className={`${ isDark ? "bg-white/2 border-white/10" : "bg-neutral-300 !text-[#2d3748] border-black/30" } backdrop-blur-md border text-white rounded-2xl shadow-lg h-full w-full`}>
