...
const NoteItem = ({ note, setEditNoteId, setDisplayEditNote }: NoteItemProps) => {
    ...
    return (
        <div
            ...
            className={`w-full py-[18px] px-[22px] cursor-pointer transition duration-80 ease-in-out border ${activeNoteId && activeNoteId === note._id ? (isDark ? "border-white/50" : "!border-black/70") : ("border-transparent")} ${isDark ? "hover:bg-white/6 bg-white/4" : "hover:bg-neutral-100 bg-white !border-black/10"} rounded-xl border-box shadow-md`}
