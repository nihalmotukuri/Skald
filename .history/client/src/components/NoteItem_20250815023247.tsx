...
const NoteItem = ({ note, setEditNoteId, setDisplayEditNote }: NoteItemProps) => {
    ...
    const { isDark } = useSelector((store: RootState) => store.the)
    ...
    return (
        <div
            ...
            className={`w-full py-[18px] px-[22px] cursor-pointer transition duration-80 ease-in-out ${activeNoteId && activeNoteId === note._id ? "border border-white/50" : "border border-transparent"} hover:bg-white/4 bg-white/4 rounded-xl border-box`}
