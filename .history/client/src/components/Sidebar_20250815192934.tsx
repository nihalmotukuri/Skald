        ...
        <ul className="mt-[18px]">
          {features.map((f) => (
            <li
              key={f.name}
              onClick={() => navigate(f.path)}
              className={`flex items-center gap-3 p-2 rounded-lg cursor-pointer text-[18px] my-[10px] ${isDark ? "hover:bg-[#0f111a5f]" : "hover:bg-[#eae5f7]" } ${isActive(f.path) && isDark ? "bg-[#0f111a80]" : ""} ${isActive(f.path) && !isDark ? "bg-[#eae5f7] !border-black/30" : ""}`}
            >
