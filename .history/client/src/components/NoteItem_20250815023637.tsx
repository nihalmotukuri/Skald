...
const NoteItem = ({ note, setEditNoteId, setDisplayEditNote }: NoteItemProps) => {
    ...
    return (
        <div
            ...
            className={`w-full py-[18px] px-[22px] cursor-pointer transition duration-80 ease-in-out border ${activeNoteId && activeNoteId === note._id ? (isDark ? "border-white/50" : "") : ("border border-transparent")} ${isDark ? "hover:bg-white/6 bg-white/4" : "hover:bg-neutral-200 bg-neutral-200/60"} rounded-xl border-box`}
