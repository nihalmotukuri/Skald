...
const AddTaskDialog = ({ setDisplayAddTask }: AddTaskDialogProps) => {
    ...
    return (
        <section className={`fixed top-0 left-0 z-999 ${isDark ? "bg-black/40" : "bg-gray-300/30" } h-full w-full flex justify-center items-center`}>
            <form
                onSubmit={onAddTask}
                className={`${isDark ? "bg-white/6 border-white/10" : "bg-black/4 border-black/20"} backdrop-blur-xl border rounded-md p-[24px] w-[420px] flex flex-col gap-[18px] relative`}
            >
                <div
                ...
                <div className='flex flex-col gap-2'>
                    ...
                    <input
                        className={`${isDark ? "bg-white/5" : "bg-neutral-100/20 !border-black/40"} backdrop-blur-md border rounded-md text-[14px] px-[12px] py-[6px]`}
                        type="text"
                        name="title"
                ...
                <div className='flex flex-col gap-2'>
                    ...
                    <textarea
                        name="description"
                        className={`${isDark ? "bg-white/5" : "bg-neutral-100/20 !border-black/20"} backdrop-blur-md border rounded-md text-[14px] px-[12px] py-[6px] resize-none`}
                        rows={5}
                ...
                <div className='flex justify-between'>
                    <div className='flex flex-col gap-2'>
                        ...
                        <input
                            className='bg-white/5 backdrop-blur-md border border-white/10 rounded-md text-[14px] px-[12px] py-[6px]'
                            type="date"
                            name="date"
