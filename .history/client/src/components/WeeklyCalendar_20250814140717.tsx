...
const WeeklyCalendar: React.FC = () => {
  ...
  return (
    <div className={`${isDark ? "text-gray-300 border-white/20 bg-white/6" : "!text-black/60 border-black/20 bg-neutral-400/50"} backdrop-blur-md border text-white rounded-2xl h-[210px] shadow-lg w-full p-[24px]`}>
      <div className="flex items-center justify-center mb-6">
        <h2 className="text-xl font-bold tracking-wider">{headerDateString}</h2>
      </div>

      <div className={`${isDark ? "bg-white/4" : "bg-black/10"} p-4 rounded-xl`}>
        <div className="grid grid-cols-7 text-center">
          {dayNames.map((day) => (
            <div key={day} className={`text-sm font-medium mb-3 ${isDark ? "text-slate-400 mb-3" : "text-black/60" }`}>
              {day}
            </div>
          ))}
