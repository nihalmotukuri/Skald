...
const TaskCompletionChart = ({ completed = 7, total = 10 }) => {
  ...
  const renderCenterLabel = () => (
    <foreignObject x="30" y="60" width="140" height="80">
      <div className="flex flex-col items-center justify-center w-full h-200px text-white font-mono">
        <span className="text-[16px] text-slate-400 tracking-wide">PROGRESS</span>
        <span className="text-5xl font-bold">{percentage}%</span>
        <span className="text-[14px] mt-1 text-slate-400">{completed} / {total} tasks</span>
      </div>
    </foreignObject>
  );
