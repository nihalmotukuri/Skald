...
const SelectPriority = ({ selectPriority, currentPriority }: Props) => {
  ...
  const { user } = useSelector((store: RootState) => store.themeStore)
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button className={`${isDark ? "bg-white/5 border-white/10" : "bg-neutral-100/20 !border-black/30"} hover:bg-white/10 backdrop-blur-md border border-white/10 rounded-md shadow-lg text-[12px] px-[12px] w-[84px] py-[6px] cursor-pointer`}>
