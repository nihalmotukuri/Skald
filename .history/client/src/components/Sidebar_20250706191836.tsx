...
const Sidebar = () => {
  ...
  const openSidebar = () => (
    <motion.div
      className="bg-[#1b1d26] min-w-[280px] rounded-[18px] p-[8px] flex flex-col justify-between max-h-full"
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
        ...
        <ul className="px-[12px] mt-[18px]">
          {features.map((f) => (
            <li
              ...
              className={`flex items-center gap-3 py-2 px-3 rounded-lg cursor-pointer text-[18px] my-[10px] hover:bg-[#0f111a5f] ${
                isActive(f.path) ? "bg-[#0f111a80]" : ""
              }`}
            >
  ...
  const closeSidebar = () => (
    <motion.div
      className="bg-[#1b1d26] rounded-[12px] flex flex-col justify-between"
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      transition={{ duration: 0.6, ease: "easeOut" }}
    >
