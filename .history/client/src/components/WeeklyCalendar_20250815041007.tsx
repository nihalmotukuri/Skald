...
const WeeklyCalendar: React.FC = () => {
  ...
  return (
    <div className={`${isDark ? "text-gray-300 border-white/10 bg-white/2" : "text-[#2d3748] border-black/20 bg-neutral-200"} backdrop-blur-md border rounded-2xl h-[210px] shadow-lg w-full p-[24px]`}>
      <div className="flex items-center justify-center mb-6">
        <h2 className="text-xl font-bold tracking-wider">{headerDateString}</h2>
      </div>

      <div className={`${isDark ? "bg-white/4" : "bg-neutral-100"} p-4 rounded-xl shadow-lg`}>
        <div className="grid grid-cols-7 text-center">
          {dayNames.map((day) => (
            <div key={day} className={`text-sm font-medium mb-3 ${isDark ? "text-slate-400 mb-3" : "text-white/80"}`}>
              {day}
            </div>
          ))}
