  ...
  const openSidebar = () => (
    ...
    >
      <div>
        ...
        <ul className="px-[12px] mt-[18px] gap-[12px]">
          {features.map((f) => (
            <li
              key={f.name}
              onClick={() => navigate(f.path)}
              className={`flex items-center gap-3 py-2 px-3 rounded-lg cursor-pointer text-[18px] hover:bg-[#0f111a5f] ${isActive(f.path) ? "bg-[#0f111a80]" : ""
                }`}
            >
