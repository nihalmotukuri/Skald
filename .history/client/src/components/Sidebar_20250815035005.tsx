...
const Sidebar = ({ user }: {user: User}) => {
  ...
  const closeSidebar = () => (
    <motion.div
      className={`rounded-[12px] flex flex-col justify-between backdrop-blur-md border rounded-2xl shadow-lg ${isDark ? "bg-white/5 border-white/10" : "bg-neutral-300 text-[#2d3748] border-black/30"}`}
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      transition={{ duration: 0.6, ease: "easeOut" }}
    >
        ...
        <ul className="mt-[18px]">
          {features.map((f) => (
            <li
              key={f.name}
              onClick={() => navigate(f.path)}
              className={`flex items-center gap-3 py-2 px-3 rounded-lg cursor-pointer text-[18px] my-[10px] ${isDark ? "hover:bg-[#0f111a5f]" : "hover:bg-neutral-200" } ${isActive(f.path) && isDark ? "bg-[#0f111a80]" : ""} ${isActive(f.path) && !isDark ? "bg-neutral-300" : ""}`}
