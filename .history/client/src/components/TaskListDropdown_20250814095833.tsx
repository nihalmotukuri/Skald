...
const TaskListDropdown = ({ selectedTaskList, setSelectedTaskList }: TaskListDropdownProps) => {
  ...
  const [onHover, setOnHover] = useState<[boolean, number]>([ display: false, index: null ])

  const onTaskListDelete = (e) => {
    e.stopPropagation()
        ...
        <DropdownMenuGroup>
          ...
          {user?.taskList?.map((tL: string, index) => (
            ...
            >
              ...
              <div 
                className={`absolute right-0 top-0 p-2 rounded-2xl hover:bg-[0f111a] ${onHover ? null : 'hidden'}`}
                onClick={onTaskListDelete}
                >
                <RxCross1 />
              </div>
            </DropdownMenuItem>
