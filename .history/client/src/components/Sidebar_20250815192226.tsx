...
const Sidebar = ({ user }: {user: User}) => {
  ...
  const openSidebar = () => (
    <motion.div
      className={`min-w-[260px] rounded-[18px] p-[8px] flex flex-col justify-between max-h-full ${isDark ? "bg-white/5 border-white/10" : "bg-white text-[#2d3748] border-black"} backdrop-blur-md border rounded-2xl shadow-2xl`}
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
        ...
        <ul className="px-[12px] mt-[18px]">
          {features.map((f) => (
            <li
              ...
              className={`flex items-center gap-3 py-2 px-3 rounded-lg cursor-pointer text-[18px] my-[8px] ${isDark ? "hover:bg-[#0f111a5f" : "hover:bg-[#eae5f7]" } ${isActive(f.path) && isDark ? "bg-[#0f111a80]" : ""} ${isActive(f.path) && !isDark ? "bg-[#eae5f7]" : ""}`}
            >
