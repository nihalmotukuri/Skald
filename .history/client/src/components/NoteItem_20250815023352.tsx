...
const NoteItem = ({ note, setEditNoteId, setDisplayEditNote }: NoteItemProps) => {
    ...
    return (
        <div
            ...
            className={`w-full py-[18px] px-[22px] cursor-pointer transition duration-80 ease-in-out ${activeNoteId && activeNoteId === note._id ? "border border-white/50" : "border border-transparent"} ${isDark ? "hover:bg-white/6 bg-white/4" : "hover: bg-black/4"} rounded-xl border-box`}
