...
const AddTaskDialog = ({ setDisplayAddTask }: AddTaskDialogProps) => {
    ...
    return (
        <section className={`fixed top-0 left-0 z-999 ${isDark ? "bg-black/40" : "bg-gray-300/30" } h-full w-full flex justify-center items-center`}>
            <form
                onSubmit={onAddTask}
                className={`${isDark ? "bg-white/6 border-white/10" : "bg-black/4 border-black/10"} shadow-lg backdrop-blur-xl border rounded-md p-[24px] w-[420px] flex flex-col gap-[18px] relative`}
            >
                <div
