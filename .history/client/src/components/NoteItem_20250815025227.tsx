...
const NoteItem = ({ note, setEditNoteId, setDisplayEditNote }: NoteItemProps) => {
    ...
    return (
        <div
            ...
            className={`w-full py-[18px] px-[22px] cursor-pointer transition duration-80 ease-in-out border ${activeNoteId && activeNoteId === note._id ? (isDark ? "border-white/50" : "border-black/50") : ("border-transparent")} ${isDark ? "hover:bg-white/6 bg-white/4" : "hover:bg-neutral-200 bg-neutral-200/60"} rounded-xl border-box`}
        ...
        >
            ...
            <div className="relative">
                ...
                <div className={`absolute z-1 top-[-4px] right-0 flex gap-3 ${displayBtns ? "block" : "hidden"}`}>
                    <div
                        className={`p-2 ${isDark ? "bg-white/10 hover:border-white/" : "bg-black/10 hover:border-black/40"} rounded-md hover:bg-transparent border border-transparent`}
