...
const DisplayNote = ({ fullscreen, setFullscreen }: DisplayNoteProps) => {
    ...
    return (
        <div className={`overflow-y-auto ${isDark ? "bg-white/4" : "bg-neutral-200/60"} p-1`}>
            <div
                className={`sticky top-0 left-0 w-[34px] p-2 text-lg ${isDark ? "hover:bg-white/10": "hover:bg-black/14"} rounded-[6px] cursor-pointer`]
                onClick={onCloseNote}
            >
                <RxCross1 />
            </div>
            ...
            <div
                className={`absolute top-[2px] right-[6px] w-[34px] p-2 text-lg ${isDark ? "hover:bg-white/10": "hover:bg-black/14"} mt-[2px] ml-[4px] rounded-[6px] cursor-pointer`}
