...
const Assistant = () => {
  ...
  return (
    <div className={`relative col-span-1 row-span-1 grid grid-cols-[1fr] grid-rows-[1fr] ${isDark ? "bg-white/2 border-white/6" : "bg-neutral-200 border-black/30"} backdrop-blur-md border rounded-md shadow-lg p-[24px] overflow-y-auto mt-4`}>
          ...
          {convo.map(c => (
            c.id % 2 == 0
              ? (
                ...
                >
                  <LuBot className={`text-[22px] ${isDark ? "bg-white/8 text-white" : "bg-neutral-300"} size-auto p-[8px] rounded-3xl`} />

                  <p className={`w-full ${isDark ? "text-slate-200 pt-[6px]" : "text-blue-95040"}`}>
                    <ReactMarkdown>
                      {c.message}
                    </ReactMarkdown>
                  </p>
