...
const Dashboard = () => {
  ...
  const renderDashboard = () => (
    <>
      <div className='col-span-2 px-3 pt-3 pb-7'>
        ...
        {notes.length > 0
          ? (
            <div
              className={`relative ${isDark ? "bg-white/2 border-white/10 hover:bg-white/4" : "bg-white !text-[#2d3748] hover:bg-neutral-100"} backdrop-blur-md border text-white rounded-2xl shadow-xl p-[20px] mb-5 cursor-pointer`}
      ...
      <div className='col-span-1'>
        ...
        <div className={`${notes.length > 0 ? "h-[721px]" : "h-[456px]"} mt-5 ${isDark ? "text-white border-white/10 bg-white/2" : "!text-[#2d3748] bg-white"} backdrop-blur-md border rounded-2xl shadow-lg py-[20px] px-[26px] grid grid-rows-[44px_1fr] shadow-lg`}>
            ...
            <ul className='h-full flex flex-col gap-2 overflow-y-auto relative'>
              {tasksThisWeek.length > 0
                ? tasksThisWeek.map(ct => (
                  <li className={`p-3 ${isDark ? "bg-white/6" : "bg-[#f0f1f5]"} rounded-lg shadow-md`} key={ct._id}>
                    <p className='mb-2'>{ct.title}</p>
                    <div className='flex justify-between'>
                      <span className={`text-[12px] py-[1px] px-[6px] rounded-sm text-gray-400 border ${isDark ? "border-white/20" : "border-black/20 bg-[#373e4e]"}`}>
                        {ct.taskList}
                      </span>
