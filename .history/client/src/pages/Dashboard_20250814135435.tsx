    ...
    <>
      <div className='col-span-2'>
        ...
        {notes.length > 0
          ? (
            <div
              className={`relative ${ isDark ? "bg-white/2 border-white/10" : "bg-neutral-300 !text-[#2d3748] border-black/30" } backdrop-blur-md border text-white rounded-2xl shadow-lg p-[20px] mb-5 cursor-pointer hover:bg-white/4`}
              onClick={() => onLatestNote(notes.at(-1)?._id ?? '')}
            >
              <h2 className='text-[18px] pb-2 truncate'>{notes.at(-1)?.title}</h2>
