  ...
  const renderDashboard = () => (
    <>
      <div className='col-span-2'>
        <div className='w-full h-[340px] grid grid-cols-[repeat(2,1fr)] gap-5 mb-5'>
          <div className='grid grid-cols-[repeat(2,1fr)] grid-rows-[repeat(9,1fr)] gap-3'>
            <div className={`col-1 row-span-5 ${ isDark ? "bg-white/2 border-white/10" : "bg-neutral-300 !text-[#2d3748] border-black/30" } backdrop-blur-md border  text-white rounded-xl p-[18px] flex flex-col justify-between`}>
            ...
            <div className={`col-1 row-span-4 ${ isDark ? "bg-white/2 border-white/10" : "bg-neutral-300 !text-[#2d3748] border-black/30" } backdrop-blur-md border text-white rounded-xl p-[18px] flex flex-col justify-between`}>
            ...
            <div className={`col-2 row-start-1 row-end-5 ${ isDark ? "bg-white/2 border-white/10" : "bg-neutral-300 !text-[#2d3748] border-black/30" } backdrop-blur-md border text-white rounded-xl p-[18px] flex flex-col justify-between`}>
            ...
            <div className={`col-2 row-start-5 row-end-10 ${ isDark ? "bg-white/2 border-white/10" : "bg-neutral-300 !text-[#2d3748] border-black/30" } backdrop-blur-md border text-white rounded-xl p-[18px] flex flex-col justify-between`}>
        ...
        {notes.length > 0
          ? (
            <div
              className={`relative ${ isDark ? "bg-white/2 border-white/10 hover:bg-white/4" : "bg-neutral-300 !text-[#2d3748] border-black/30 hover:bg-neutral-200" } backdrop-blur-md border text-white rounded-2xl shadow-lg p-[20px] mb-5 cursor-pointer`}
            ...
            >
              ...
              alt="Note Image"
              />
              <p className={`absolute bottom-5 right-5 text-[14px] px-[8px] py-1 rounded-2xl border ${isDark ? "text-gray-300 border-white/20 bg-white/6" : "text-white border-black/20 bg-neutral-400/50"} `}>✨ Latest note</p>
            </div>
          ) : null
        }
      ...
      <div className='col-span-1'>
        ...
        <div className={`${notes.length > 0 ? "h-[693px]" : "h-[456px]"} mt-5 ${isDark ? "text-white border-white/10 bg-white/2" : "!text-black/60 border-black/20 bg-neutral-400/50"} backdrop-blur-md border rounded-2xl shadow-lg py-[20px] px-[26px] grid grid-rows-[44px_1fr]`}>
          <p className="text-white font-bold text-[18px] mb-[16px]">
            Completed This Week
          </p>
            ...
            <ul className='h-full flex flex-col gap-2 overflow-y-auto relative'>
              {tasksThisWeek.length > 0
                ? tasksThisWeek.map(ct => (
                  <li className={`p-3 ${isDark ? "bg-white/6" : "bg-[#0f111a]"} rounded-lg`} key={ct._id}>
